...
This guide describes how `TenantContext` moves beyond the initial HTTP request and which propagation boundaries are handled by each Quarkus Multitenancy module.
...
Tenant context should not be treated as a global or thread-local value. Code crossing an asynchronous or messaging boundary must use the propagation mechanism appropriate for that boundary.
...
The callback can outlive the temporary tenant binding. Use an asynchronous context-propagation mechanism appropriate to the framework executing that work instead of relying on `TenantContextRunner` after the callback returns.

== Kafka tenant propagation

The optional Kafka module propagates the current tenant through Kafka record headers when using SmallRye Reactive Messaging with the Kafka connector.
...
Kafka record headers are an external trust boundary. Incoming tenant identifiers are validated before they are published to `TenantContext`.
...
The defaults match the conservative HTTP tenant-id policy: validation is enabled, the maximum length is `64`, and the tenant must match `[A-Za-z0-9_-]+` in full.
...
| `true`
| Enables validation of untrusted incoming Kafka tenant identifiers.

| `quarkus.multi-tenant.messaging.kafka.tenant-id.max-length`
| `64`
| Maximum accepted incoming tenant-id length.

| `quarkus.multi-tenant.messaging.kafka.tenant-id.pattern`
| `[A-Za-z0-9_-]+`
| Regular expression an incoming tenant identifier must match in full.
|===
...
| Asynchronous result returned from background work
| Do not rely on `TenantContextRunner`; use the context-propagation mechanism of the asynchronous framework.
...
Keep the tenant boundary explicit when moving work outside a request. `TenantContextRunner` is intended for synchronous background callbacks, while Kafka propagation is intended for Kafka connector boundaries; neither is a general-purpose global context store.