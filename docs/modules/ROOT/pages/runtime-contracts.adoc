...
[IMPORTANT]
====
The current API does not define a deterministic order between multiple custom `TenantResolver` beans. Do not rely on CDI discovery/iteration order when two custom resolvers can both handle the same request. Keep custom resolvers mutually exclusive until explicit resolver priority support is available.
====
