...
| `400`
| HTTP status used when a resolved tenant id fails the length or pattern policy. Defaults to `400` (malformed request input). Does not affect the `401` returned for an authentication-related rejection. Set to `401` to restore the previous behaviour.
|===
