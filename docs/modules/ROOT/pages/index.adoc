...
| Yes
| Requires `quarkus-smallrye-context-propagation` (see below). The
`ManagedExecutor` captures and restores the context around each task.
...
there throws `ContextNotActiveException`. Note that a Mutiny pipeline that
`emitOn` a raw executor still propagates, because Mutiny wraps the executor when
context propagation is present; the unsupported case is handing work to a raw
executor outside a Mutiny pipeline or `ManagedExecutor`.
|===
...
`ManagedExecutor` and add the dependency that provides it:
...
The dependency is necessary but not sufficient: propagation only happens on the
`ManagedExecutor` itself. Submitting to a raw executor, or to the injectable
core `ExecutorService`, still loses the context even when the dependency is
present.
