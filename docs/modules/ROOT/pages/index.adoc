...
* Optional Kafka header propagation across Reactive Messaging boundaries.
...
For Kafka messaging propagation:
...
== Kafka messaging propagation

The optional Kafka module copies the current `TenantContext` value to an outgoing Kafka record header and restores it for an incoming handler.
It uses SmallRye Reactive Messaging interceptors and keeps the incoming request scope alive until the message is acknowledged or negatively acknowledged, including asynchronous handlers.
Propagation is restricted to channels configured with the `smallrye-kafka` connector; other connectors and their application interceptors remain unchanged.

Enable Quarkus Messaging request-scope lifecycle support at build time:
...
The module defaults to the `X-Tenant` Kafka header. To choose another header:

[source,properties]
----
quarkus.multi-tenant.messaging.kafka.header-name=tenant-key
----

Explicit outgoing Kafka metadata takes precedence over the tenant in `TenantContext`.
When no tenant is available, messages continue without a tenant header by default. Strict applications can reject missing tenants in either direction:
...
Incoming header values are decoded as strict UTF-8 and validated before they reach `TenantContext`.
Blank, malformed, overlong, or pattern-mismatched identifiers are rejected.
Rejection nacks and drops only the affected message, allowing the channel to continue processing subsequent records.
Applications can add a CDI bean implementing `KafkaTenantValidator` for domain checks such as verifying the identifier against a tenant registry.

When the HTTP module's tenant-id policy is configured, Kafka propagation inherits that policy by default.
Any explicitly configured `quarkus.multi-tenant.messaging.kafka.tenant-id.*` value overrides the corresponding HTTP setting.

Main Kafka options include:
...
| `true`
| Enables or disables Kafka tenant propagation when the optional module is installed.
...
| `false`
| Rejects an incoming message that has no tenant header. When disabled, the handler runs with an empty `TenantContext`.

| `quarkus.multi-tenant.messaging.kafka.fail-on-missing-outgoing-tenant`
| `false`
| Rejects an outgoing send when neither explicit metadata nor a current tenant is available.

| `quarkus.multi-tenant.messaging.kafka.tenant-id.validation-enabled`
| HTTP setting, or `true`
| Validates tenant identifiers against the configured length and pattern.
...
| HTTP setting, or `[A-Za-z0-9_-]+`
| Pattern that a tenant identifier must match in full.
|===
