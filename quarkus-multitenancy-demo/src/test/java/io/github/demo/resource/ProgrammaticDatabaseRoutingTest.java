...
class ProgrammaticDatabaseRoutingTest {

    @Test
    void headerRoutesToEachProgrammaticPool() {
        assertTenantData(RestAssured.given().header("X-Tenant", "tenant1"), "/api/users",
                "tenant1@example.com", "tenant2@example.com");
        assertTenantData(RestAssured.given().header("X-Tenant", "tenant2"), "/api/users",
                "tenant2@example.com", "tenant1@example.com");
    }

    @Test
    void cookieRoutesToProgrammaticPool() {
        assertTenantData(RestAssured.given().cookie("tenant_cookie", "tenant2"), "/api/users",
                "tenant2@example.com", "tenant1@example.com");
    }

    @Test
    void pathRoutesToProgrammaticPool() {
        assertTenantData(RestAssured.given(), "/api/users/path/tenant2",
                "tenant2@example.com", "tenant1@example.com");
    }

    @Test
    void defaultTenantRoutesToProgrammaticPool() {
        assertTenantData(RestAssured.given(), "/api/users",
                "tenant1@example.com", "tenant2@example.com");
    }

    @Test
    void unknownTenantIsRejectedBeforePoolCreation() {
        RestAssured.given().header("X-Tenant", "missing")
                .when().get("/api/users")
                .then().statusCode(404);
    }
